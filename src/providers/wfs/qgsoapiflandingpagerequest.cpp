...
void QgsOapifLandingPageRequest::processReply()
{
  ...
  try
  {
    ...
    if ( j.is_object() && j.contains( "links" ) )
    {
      const auto links = j["links"];
      if ( links.is_array() )
      {
        bool authoritativeApiUrlFound = false;
        bool authoritativeCollectionsUrlFound = false;
        ...
        for ( const auto &link : links )
        {
               ...
               link.contains( "rel" ) )
          {
            ...
            std::string typeStdStr;
            if ( link.contains( "type" ) )
            {
              const auto type = link["type"];
              if ( type.is_string() )
              {
                typeStdStr = type.get<std::string>();
              }
            }
            if ( href.is_string() && rel.is_string() )
            {
              auto relStdStr = rel.get<std::string>();
              auto hrefStr = QString::fromStdString( href.get<std::string>() );
              if ( relStdStr == "service-desc"
#ifndef REMOVE_SUPPORT_DRAFT_VERSIONS
                   || relStdStr == "service"
#endif
                 )
              {
                if ( typeStdStr == "application/vnd.oai.openapi+json;version=3.0" ||
                     typeStdStr == "application/openapi+json;version=3.0" )
                {
                  mApiUrl = hrefStr;
                  authoritativeApiUrlFound = true;
                }
                else if ( !authoritativeApiUrlFound )
                {
                  mApiUrl = hrefStr;
                }
              }
              else if ( relStdStr == "data"
#ifndef REMOVE_SUPPORT_DRAFT_VERSIONS
                        || relStdStr == "collections"
#endif
                      )
              {
                if ( typeStdStr == "application/json" )
                {
                  mCollectionsUrl = hrefStr;
                  authoritativeCollectionsUrlFound = true;
                }
                else if ( !authoritativeCollectionsUrlFound )
                {
                  mCollectionsUrl = hrefStr;
                }
              }
